...
class Users_model extends CI_Model {
	...
	public function get_user ($member_id=0) {		
		$this->db->select ('M.*, B.batch_id, B.batch_name'); 
		$this->db->from ('members M');
        $this->db->join ('coaching_course_batch_users BU', 'M.member_id=BU.member_id', 'left');
        $this->db->join ('coaching_course_batches B', 'BU.batch_id=B.batch_id', 'left');
	...
	*/
	...
	public function add_user_to_batch ($coaching_id=0, $member_id=0, $batch_id=0) {
		...
		if  ($sql->num_rows () == 0 ) { 
			...
			$this->db->where ($data);
			$sql = $this->db->update ('coaching_course_batch_users');
		}

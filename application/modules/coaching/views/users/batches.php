		...
		foreach ($all_batches as $row) { 
			...
			<li class="list-group-item d-flex flex-sm-row flex-column">
				<div class="flex-grow-1 my-auto">
					<h4><a href="<?php echo site_url ('coaching/users/batch_users/'.$coaching_id.'/'.$row['batch_id']); ?>"><?php echo $row["batch_name"];?></a></h4>
				...
				<div class="flex-shrink-1 px-sm-3 my-auto">
					...
					?>
					<div class="btn-group">
						<a href="<?php echo site_url ('coaching/users/batch_users/'.$coaching_id.'/'.$row['batch_id']); ?>" class="btn btn-primary btn-sm"><?php echo $num; ?> Users</a>
						<a href="<?php echo site_url('coaching/users/create_batch/'.$coaching_id.'/'.$row["batch_id"]); ?>" class="btn btn-info btn-sm"><i class="fa fa-edit"></i><span class="d-none d-lg-inline ml-2">Edit</span></a>	
						<a href="javascript:void(0);" onclick="show_confirm ('Are you sure delete this batch?', '<?php echo site_url('coaching/user_actions/delete_batch/'.$coaching_id.'/'.$row["batch_id"]); ?>')" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i><span class="d-none d-lg-inline ml-2">Delete</span></a>
			...
			<?php } 
				...
				<li class="list-group-item ">
					<div class="text-danger">No classes found. <?php echo anchor ('coaching/users/create_batch/'.$coaching_id, 'Create New Class', ['class'=>'btn btn-outline-primary']); ?></div>
				</li>
				<?php
			}
			?>

...
class Users extends MX_Controller {
	...
	public function __construct () {
	    ...
	    $models = ['users_model', 'coaching_model', 'subscription_model', 'enrolment_model', 'classes_model'];
	...
	*/
	public function index ($coaching_id=0, $role_id=0, $status='-1', $batch_id=0, $sort=SORT_ALPHA_ASC) {
		...
		$data['batch_label'] = 'All Classes';
		if (! empty($batches)) {
			foreach ($batches as $batch) {
				if ($batch_id == $batch['batch_id']) {
					$data['batch_label'] = $batch['batch_name'];
				}
			}
		}
	...
	*/
	...
	public function import ($coaching_id=0, $role_id=USER_ROLE_STUDENT, $batch_id=0) { 
	 ...
	 * CLASSES
	 */
	public function classes ($coaching_id=0) {
		/* Breadcrumbs */ 
		$data['bc'] = array ('Users'=>'coaching/users/index/'.$coaching_id);
		...
		$data['classes'] = $this->classes_model->get_classrooms ($coaching_id);
		//$data['script'] = $this->load->view ('settings/scripts/classrooms', $data, true);
		$this->load->view(INCLUDE_PATH  . 'header', $data);
		$this->load->view('users/classes', $data);
		$this->load->view(INCLUDE_PATH  . 'footer', $data);	    
	}

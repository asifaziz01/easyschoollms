...
class Courses extends MX_Controller {
	...
	public function index ($coaching_id=0, $cat_id='-1') {
		...
		$data['batches']	 	= $batches = $this->users_model->get_batches ($coaching_id);
		if ($is_admin) {
			$data['courses'] = $this->courses_model->courses ($coaching_id, $cat_id);
			$data['toolbar_add_button'] = array (
				'<i class="fa fa-plus-circle"></i> New Subject' => 'coaching/courses/create/'.$coaching_id.'/'.$cat_id
			);
			$data['toolbar_buttons'] = array (
				'<i class="fa fa-plus-circle"></i> Classes' => 'coaching/users/batches/'.$coaching_id,
				'<i class="fa fa-plus-circle"></i> New Class' => 'coaching/courses/create_category/'.$coaching_id,
			);
		} else {
	...
	public function create ($coaching_id = 0, $cat_id = -1, $course_id = 0) {
		...
		$data['sub_title'] = ($this->router->fetch_method() == "create") ? 'Create New Subject' : 'Edit Subject';
		$data['submit_label'] = ($this->router->fetch_method() == "create") ? 'Create' : 'Update';
		$data['submit_title'] = ($this->router->fetch_method() == "create") ? 'Create New Subject' : 'Update This Subject';
		$data['cat_id'] = $cat_id;
		$data['coaching_id'] = $coaching_id;
		...
		$data['bc'] = ($this->router->fetch_method() == "create") ? array('Subjects' => 'coaching/courses/index/' . $coaching_id) : array('Manage Subject' => 'coaching/courses/manage/'.$coaching_id.'/'.$course_id);

...
class User_actions extends MX_Controller {
	...
	*/	
	...
	public function create_account ($coaching_id=0, $role_id=0, $member_id=0) {
		...
		$this->form_validation->set_rules ('login', 'Enrolment No', 'required|trim');
		...
		if ($this->form_validation->run () == true) {
			...
			} else if ($this->users_model->check_unique ($login, 'login', $member_id) == true) {
		...
		if ( ! $this->upload->do_upload()) {
			...
			if (($handle = fopen (base_url($file), "r")) !== FALSE) {
			    ...
			    foreach ($result as $index => $row) {
					...
					if ($users['login'] == '' || $users['primary_contact'] == '' || $users['first_name'] == '' ) {
						$count_error++;
					/*} else if ($num_users >= $free_users) {
						//$this->output->set_content_type("application/json");
						$this->output->set_output(json_encode(array('status'=>false, 'error'=>'User limit reached. You can create a maximum of '.$free_users.' user accounts in Free Subscription plan. Upgrade your plan to create more users' )));*/
					} else if ($this->users_model->check_unique ($users['login'], 'login') == true) {
						$count_error++;
					} 
					...
					if ($count_error == 0) {
						...
						$message = $i  . ' users imported successfully.';
					} else {
						$message = $i  . ' users imported successfully.';
						$message .= $count_error. ' records were skipped due to insufficient data/duplicate enrolment number.';
					}
			    }

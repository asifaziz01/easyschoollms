...
class Login_actions extends MX_Controller {
	...
	public function register () {
		$this->form_validation->set_rules('login', 'Enrolment No', 'required|trim', ['required'=>'Please enter Enrolment No']); 
		...
		if ( $this->form_validation->run() == true) {
			...
			if (! $coaching) {
			...
			} else if ($this->users_model->check_unique ($login, 'login') == true) {
				// Check if already exists
				$this->output->set_content_type("application/json");
				$this->output->set_output(json_encode(array('status'=>false, 'error'=>'There is already an account with this enrolment number. Try Sign-in instead' )));
			} else {
				...
				if ($coaching['email'] != '') {					
					...
					$subject = 'New Registration';
						$email_message = 'A new user <strong>'.$user_name.'</strong> has registered in <strong>'.$coaching_name. '</strong>. ';
				...
				if ($status == USER_STATUS_UNCONFIRMED) {
					...
					$data['url'] = site_url ('login/login/index');
					$data['login'] = $login;
	...
	public function reset_password () {
		$this->form_validation->set_rules('login', 'Enrolment Number', 'required|trim');
		$this->form_validation->set_rules('access_code', 'Access Code', 'required|trim');
		...
		if ($this->form_validation->run () == true) {
			...
			if ($user == false) {
				$this->output->set_content_type("application/json");
				$this->output->set_output(json_encode(array('status'=>false, 'error'=>'Please check your enrolment number and try again') ));
			} else {
